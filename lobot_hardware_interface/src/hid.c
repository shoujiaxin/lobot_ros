...
static int get_device_string(hid_device *dev, enum device_string_id key,
  ...
  if (udev_dev) {
    ...
    if (hid_dev) {
      ...
      if (bus_type == BUS_BLUETOOTH) {
        ...
        parent = udev_device_get_parent_with_subsystem_devtype(udev_dev, "usb",
                                                               "usb_device");
        if (parent) {
          const char *str;
...
struct hid_device_info HID_API_EXPORT *hid_enumerate(
  ...
  udev_list_entry_foreach(dev_list_entry, devices) {
    ...
    struct udev_device
        *intf_dev; /* The device's interface (in the USB sense). */
    unsigned short dev_vid;
    unsigned short dev_pid;
    char *serial_number_utf8 = NULL;
    char *product_name_utf8 = NULL;
    ...
    if ((vendor_id == 0x0 || vendor_id == dev_vid) &&
      ...
      switch (bus_type) {
        case BUS_USB:
          ...
          cur_dev->manufacturer_string = copy_udev_string(
              usb_dev, device_string_names[DEVICE_STRING_MANUFACTURER]);
          cur_dev->product_string = copy_udev_string(
              usb_dev, device_string_names[DEVICE_STRING_PRODUCT]);
  ...
  if (dev->device_handle > 0) {
    ...
    if (res < 0) {
      ...
      dev->uses_numbered_reports =
          uses_numbered_reports(rpt_desc.value, rpt_desc.size);
    }
...
size_t HID_API_EXPORT hid_read_timeout(hid_device *dev, unsigned char *data,
  ...
  if (bytes_read >= 0 && kernel_version != 0 &&
      kernel_version < KERNEL_VERSION(2, 6, 34) && dev->uses_numbered_reports) {
...
size_t HID_API_EXPORT hid_read(hid_device *dev, unsigned char *data,
                               size_t length) {
...
size_t HID_API_EXPORT hid_send_feature_report(hid_device *dev,
